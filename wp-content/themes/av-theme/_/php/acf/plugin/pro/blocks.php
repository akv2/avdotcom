...
function acf_enqueue_block_type_assets( $block_type ) {
	...
	if ( $block_type['enqueue_style'] ) {
		wp_enqueue_style( $handle, $block_type['enqueue_style'], array(), ACF_VERSION . '-' . get_field('version', 'option'), 'all' );
	}

	// Enqueue script.
	if ( $block_type['enqueue_script'] ) {
		wp_enqueue_script( $handle, $block_type['enqueue_script'], array(), ACF_VERSION . '-' . get_field('version', 'option'), true );
	}
